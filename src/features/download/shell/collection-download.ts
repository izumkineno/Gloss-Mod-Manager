...
export async function queueNexusCollectionDownloadWithSelection(
    ...
    // Wave 2：勾选后直接逐个建任务（facade.enqueue，经 queueThirdPartyModDownload 内聚，collectionId 归属壳）。
    // collection_pending 旁路已删：不落盘待下载清单，失败只记 failedFiles，不入机。
    const collectionId = `${options.gameDomain}:${options.slug}:R${revision}`;
